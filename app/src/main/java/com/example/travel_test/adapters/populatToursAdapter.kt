...
class populatToursAdapter(delegate: TravelDelegate) :
    BaseRecyclerAdapter<populatToursViewHolder, TourVO>() {
    val delegate: TravelDelegate = delegate
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): populatToursViewHolder {
        val view =
            LayoutInflater.from(parent.context).inflate(R.layout.item_popular_tours, parent, false)
